# Embeds design/app.ico into CommandCentre.exe as RT_GROUP_ICON + RT_ICON resources.
# The `deno desktop` runtime shell carries no icon resources (--icon only
# embeds into CommandCentre.dll), so the window title-bar and taskbar fall back to
# the Chrome/CEF default. Win32 UpdateResource adds the icon in place.
param(
    [string]$ExePath = "C:\projects\command-centre\dist\CommandCentre-app\CommandCentre\CommandCentre.exe",
    [string]$IcoPath = "C:\projects\command-centre\design\app.ico"
)
