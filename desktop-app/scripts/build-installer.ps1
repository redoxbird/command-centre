# Builds the Command Centre Inno Setup installer from the `deno desktop` payload.
# Pipeline: deno task build:dir (bat + payload.tar.xz) -> extract -> ISCC.
$ErrorActionPreference = "Stop"
$repo = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$payloadDir = Join-Path $repo "dist\CommandCentre"
$appDir = Join-Path $repo "dist\CommandCentre-app"
...
if (-not (Test-Path $payload)) {
    ...
    $payload = Join-Path $repo "dist\CommandCentre-msi\CommandCentre\payload.tar.xz"
}
if (-not (Test-Path $payload)) {
...
# Fresh extraction of the app payload (CommandCentre.exe + CEF runtime)
...
# into CommandCentre.dll), so the window title-bar falls back to the CEF/Chrome
# default. Embed the ico into the exe so the title bar and taskbar show it.
& (Join-Path $PSScriptRoot "embed-icon.ps1") `
    -ExePath (Join-Path $appDir "CommandCentre\CommandCentre.exe") `
...
Copy-Item -Force (Join-Path $repo "design\app.ico") (Join-Path $appDir "CommandCentre\AppIcon.ico")

# Resolve version from version.ts / deno.json for installer metadata (ensures reinstall is seen as update)
# version.ts wins over deno.json.
$version = "1.0.0"
try {
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Output "Installer: dist\CommandCentre-setup.exe"
