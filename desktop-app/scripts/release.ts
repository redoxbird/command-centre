/**
 * Release publisher for Command Centre — uploads built artifacts to R2 and records
 ...
 *   e.g. command-centre/1.0.0/CommandCentre-setup.exe
 *
    ...
    try {
      for await (const entry of Deno.readDir(dir)) {
        ...
        if (entry.isDirectory) {
          ...
          //  - dist/CommandCentre/            -> contains payload.tar.xz intermediate
          //  - dist/CommandCentre-app/        -> extracted runtime (hundreds of MB, not a release)
          //  - vendor/, locales/          -> not releases
          const base = entry.name;
          const isDistBuild = full.replace(/\\/g, "/").endsWith("/dist/CommandCentre");
          const isDistApp = full.replace(/\\/g, "/").endsWith("/dist/CommandCentre-app") || full.includes("CommandCentre-app");
          if (isDistBuild || isDistApp) continue;
          if (base === "locales" || base === "vendor") continue;
          // Only recurse one level deep for dist (keep top-level artifacts plus one subfolder like CommandCentre-msi)
          if (depth >= 1) continue;
          ...
          if (match) {
            ...
            if (entry.name === "CommandCentre.bat") continue;
            const key = full;
            if (!seen.has(key)) {
              seen.add(key);
              out.push(full);
...
function printHelp() {
  console.log(`Command Centre Release Publisher
...
Options:
  ...
  --prefix <pfx>      Override object prefix (default: command-centre or R2_OBJECT_PREFIX)
...
if (import.meta.main) {
  ...
  const objectPrefix = prefixOverride ?? getEnv("R2_OBJECT_PREFIX") ?? "command-centre";
  const bucket = bucketOverride ?? publicBucket ?? privateBucket ?? pathBucket ?? "command-centre";
  ...
  console.log(`\nCommand Centre release — v${version}${dryRun ? " (dry-run)" : ""}`);
  ...
  const newEntry: ReleaseEntry = {
    ...
    date: displayDate,
    displayDate,
    title: `Command Centre ${versionTag}`,
    channel: "stable",
    kind,
    latest: true,
    tags,
    notes,
    summary: notes.added[0] ?? `Command Centre ${versionTag}`,
    assets,
    assetsBase,
  };
